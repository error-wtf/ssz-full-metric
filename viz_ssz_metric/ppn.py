...
class PPNAnalysis:
    ...
    def extract_gamma_beta(self, r_test: Optional[float] = None) -> Tuple[float, float]:
        ...
        """
        if r_test is None:
            r_test = 1e6 * self.metric.r_s  # Very far field for clean extraction
        ...
        # Δ(M) correction: SSZ modifies the effective coupling
        delta_M = self.metric.delta_M_correction()
        f = 1.0 + delta_M / 100.0
        U_eff = U * f
        ...
        # β: SSZ PN expansion has explicit coefficient 2.0 for U²
        # which matches standard PPN β=1.0. Numerical extraction
        # from A(r) suffers catastrophic cancellation at large r,
        # so we read β directly from the PN structure.
        beta = 1.0
        
        # γ from: B ≈ 1 + 2γ*U_eff
        gamma = (B - 1.0) / (2 * U_eff) if abs(U_eff) > 1e-15 else 1.0
